...
extension UIViewController {
    ...
    func setNavigationHeader(strTitleName : String, isTabbar : Bool = false) {
        
        if #available(iOS 13.0, *) {
            let navBarAppearance = UINavigationBarAppearance()
            navBarAppearance.configureWithOpaqueBackground()
            
            navBarAppearance.titleTextAttributes = [.foregroundColor: UIColor.appMain(), .font : (UIFont(name: "Poppins-Bold" , size: 28) ?? UIFont.systemFont(ofSize: 28).withWeight(UIFont.Weight.bold))]
            
            navBarAppearance.backgroundColor = .appWhite()
        ...
        UINavigationBar.appearance().tintColor = .appMain()
        
        let label = UILabel()
        label.backgroundColor = .clear
        label.numberOfLines = 2
        label.lineBreakMode = .byWordWrapping
        label.font = UIFont(name: "Poppins-Bold", size: 28.0) ?? UIFont.systemFont(ofSize: 28).withWeight(UIFont.Weight.bold)
        ...
        if isTabbar {
            ...
            self.tabBarController?.navigationController?.navigationBar.barTintColor = .appWhite()
            self.tabBarController?.navigationController?.navigationBar.tintColor = .appMain()
            ...
            self.navigationController?.navigationBar.barTintColor = .appWhite()
            self.navigationController?.navigationBar.tintColor = .appMain()
    ...
    func setupBackButton(isTabbar : Bool = false) {
        let btnBack = UIBarButtonItem(image: #imageLiteral(resourceName: "ic_arrow"), style: .plain, target: self, action: #selector(popVC))
        if isTabbar {
